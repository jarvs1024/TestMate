<template>
  <div class="cr">
    ...
    <div v-if="health && !health.configured" class="card warn-card">
      ...
      <div class="warn-body">
        <div class="warn-t">ReviewAgent 未配置</div>
        <div class="warn-d">admin 可在 <strong>设置 → 🧪 代码检视 V2 (ReviewAgent)</strong> 填 base_url + token.<br />
    ...
    <div v-else-if="loadError" class="card warn-card">
      ...
      <div class="warn-body">
        <div class="warn-t">ReviewAgent 不可达</div>
    ...
    <div v-if="overview" class="cluster-grid">
      <!-- 被忽略的规则 (按 reason 汇总, 来自 ReviewAgent /dismissals/by-rule) -->
...
// reason 短摘要: ReviewAgent 给的 reason 可能是几百字的 "Suggestion: ... + dismiss 模板",
...
function summarizeReason(text: string): string {
  ...
  // 去掉 markdown 标记 (** ` ```) 和 ReviewAgent 注入的 "**Suggestion:**" 前缀
...
async function reloadKeepPage(opts: { silent?: boolean } = {}) {
  ...
  try {
    ...
    if (!opts.silent) {
      ElMessage.error('代码检视 V2 加载失败: ' + loadError.value);
    }
  } finally {
    loading.value = false;
